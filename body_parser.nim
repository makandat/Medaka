...
proc getDispositionFileName*(disposition: string): string =
  var p = disposition.find("Content-Disposition: form-data; name=\"")
  if p < 0:
    return ""
  else:
...
proc getDispositionChunk*(disposition: string): string =
  var p = disposition.find("Content-Type: application/octet-stream")
  if p < 0:
    return ""
  else:
...
proc getChunk*(dispositions: seq[string], name:string): string =
  var name1: string
  for d in dispositions:
    if d.isWithChunk() == true:
      name1 = d.getDispositionName()
      if name == name1:
        return d.getDispositionChunk().decodeUrl()
  return ""

# get filename of the name
proc getFileName*(dispositions: seq[string], name:string): string =
  var name1: string
  for d in dispositions:
    if d.isWithChunk() == true:
      name1 = d.getDispositionName()
